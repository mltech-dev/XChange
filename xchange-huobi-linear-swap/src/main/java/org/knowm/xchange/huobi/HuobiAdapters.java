...
public class HuobiAdapters {
	...
	public static List<Ticker> adaptAllTickers(HuobiAllTicker[] allTickers) {
		...
		return Arrays.stream(allTickers).filter(huobiTicker -> !"hb10".equals(huobiTicker.getSymbol()) // Fix on data
																										// error
																										// retrieved
																										// from api
		).map(huobiTicker -> new Ticker.Builder().currencyPair(adaptCurrencyPair(huobiTicker.getSymbol()))
				.open(huobiTicker.getOpen()).ask(huobiTicker.getAsk().getPrice()).bid(huobiTicker.getBid().getPrice())
				.askSize(huobiTicker.getAsk().getVolume()).bidSize(huobiTicker.getBid().getVolume())
				.last(huobiTicker.getClose()).high(huobiTicker.getHigh()).low(huobiTicker.getLow())
				.quoteVolume(huobiTicker.getVol()).timestamp(huobiTicker.getTs()).build()).collect(Collectors.toList());
	}
	...
	private static CurrencyMetaData adaptCurrencyMetaData(HuobiCurrencyWrapper huobiCurrencyWrapper,
			boolean isDelisted) {
	...
	private static CurrencyPairMetaData adaptPair(HuobiAssetPair pair, CurrencyPairMetaData metadata) {
		BigDecimal minQty = metadata == null ? null
		...
		return new CurrencyPairMetaData(fee, minQty, null, null, null, new Integer(pair.getAmountPrecision()),
				new Integer(pair.getPricePrecision()), null, feeTiers, null, null, true);
	}
	...
	public static Wallet adaptWallet(Map<String, HuobiBalanceSum> huobiWallet) {
		...
		for (Map.Entry<String, HuobiBalanceSum> record : huobiWallet.entrySet()) {
			try {
				...
				// It might be a new currency. Ignore the exception and continue with other
				// currency.
			}
		}
...
//      }
	...
	public static Order adaptOrder(HuobiOrder openOrder) {
		...
		order = new FuturesOrder.Builder(orderType, currencyPair).averagePrice(openOrder.getTradeAvgPrice())
				.contractCode(openOrder.getContractCode()).contractType(openOrder.getContractType())
				.cumulativeAmount(cumulativeAmount).fee(openOrder.getFee()).leverRate(openOrder.getLeverRate())
				.offset(openOrder.getOffset()).orderPriceType(openOrder.getOrderPriceType()).orderStatus(orderStatus)
				.orderType(orderType).timestamp(timestamp).originalAmount(BigDecimal.valueOf(openOrder.getVolume()))
				.price(openOrder.getPrice()).userReference(openOrder.getClientOrderId()).build();
	...
	/**
	 * Huobi currently doesn't have trade history API. We simulate it by using the
	 * orders history.
	 *
	 * @param order
	 * @return
	...
	private static UserTrade adaptTrade(Order order) {
		BigDecimal feeAmount = order.getFee();
		return new UserTrade.Builder().type(order.getType()).originalAmount(order.getCumulativeAmount())
				.currencyPair(order.getCurrencyPair()).price(order.getAveragePrice()).timestamp(order.getTimestamp())
				.id(order.getId()) // Trade id
				.orderId(order.getId()) // Original order id
				.feeAmount(feeAmount).feeCurrency(order.getCurrencyPair().counter).build();
	}
	...
	public static FundingRecord adaptFundingRecord(HuobiFundingRecord r) {
		...
		return new FundingRecord(r.getAddress(), r.getCreatedAt(), Currency.getInstance(r.getCurrency()), r.getAmount(),
				Long.toString(r.getId()), r.getTxhash(), r.getType(), adaptFundingStatus(r), null, r.getFee(), null);
	}
	 ...
	 *
	 * <p>
	 * State Description unknown On-chain transfer has not been received confirming
	 * On-chain transfer waits for first confirmation confirmed On-chain transfer
	 * confirmed for at least one block safe Multiple on-chain confirmation happened
	...
	public static String toSymbolForGetOrder(CurrencyPair pair) {
	...
	public static List<OpenPosition> adaptPosition(List<PositionResultData> positionData) {
		...
		for (PositionResultData data : positionData) {
			OpenPosition.Type type = data.getDirection().equals("buy") ? OpenPosition.Type.LONG
					: OpenPosition.Type.SHORT;
			openPositions.add(new OpenPosition(new CurrencyPair(data.getSymbol() + "/USDT"), type, data.getVolume(),
					data.getLastPrice()));
		}
