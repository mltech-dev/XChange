...
public class BitmexStreamingTradeService implements StreamingTradeService {
	...
	public BitmexStreamingTradeService(BitmexStreamingService streamingService) {
		...
		this.rawWebTransactions = streamingService.subscribeBitmexChannel("order");
	}
	...
	public Observable<Order> getOrders(CurrencyPair currencyPair, Object... args) {
		...
		return streamingService.subscribeBitmexChannel(channelName).flatMapIterable(s -> {
			...
			return Arrays.stream(bitmexOrders).filter(bitmexOrder -> bitmexOrder.getSymbol().equals(instrument))
					.filter(BitmexOrder::isNotWorkingIndicator).map(BitmexOrder::toOrder).collect(Collectors.toList());
		});
	}

	public Observable<Order> getOrderChanges() {
		return getRawExecutionReports().flatMapIterable(s -> {
			BitmexOrder[] bitmexOrders = s.toBitmexOrders();
			List<Order> orders = Arrays.stream(bitmexOrders).map(BitmexOrder::toOrder).collect(Collectors.toList());
	...
	public Observable<UserTrade> getUserTrades() {
		return getRawExecutionReports().flatMapIterable(s -> {
			...
			List<UserTrade> userTrades = Arrays.stream(bitmexOrders)
					...
					.map(BitmexOrder::toUserTrade).collect(Collectors.toList());
